...
- **Генератор CV** — три режима: «с опытом», «без опыта (junior)», «сгенерировать опыт» (с обязательным дисклеймером). На выходе ATS-friendly `.docx`. Вакансию можно дать ссылкой или текстом.
...
- **Импорт профиля** — вставьте `profile.yaml` (или JSON) или выберите файл — данные маппятся в профиль, не заполняя форму вручную.
- **Трекинг откликов** — Kanban-доска + список, настраиваемая воронка этапов, таймлайн, заметки, привязка сгенерированного CV.
- **Фиксация отправки** — канал (email/telegram/…), кому, когда — и авто-переход на этап «Отправлено».
- **Ответы и созвоны** — фиксация ответа (когда/откуда) и назначенных собеседований (тип, дата, канал, ссылка) с авто-переходом по воронке и счётчиком «через N дн/ч».
- **Приоритизация** — сортировка воронки: по этапам / ближайшие события / недавние + полоса «Скоро».
- **Напоминания** — web-push уведомления о предстоящих собеседованиях (Vercel Cron).
...
| `CRON_SECRET` | Секрет Vercel Cron (напоминания о собеседованиях) |
