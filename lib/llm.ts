...
export type VacancyLevel = "junior" | "regular"

export function vacancyLevel(text?: string | null): VacancyLevel {
  const t = (text ?? "").toLowerCase()
  const junior =
    /(стаж[её]р|стажировк|junior|intern|trainee|без опыта|опыт.{0,12}не требуется|начинающ)/i
  return junior.test(t) ? "junior" : "regular"
}
...
const SENIOR_STYLE = `Стиль резюме (обычная вакансия):
- Никаких "Ищу позицию…", "Хочу…", "Цель" — заявляй ценность, а не проси.
...
const JUNIOR_STYLE = `Стиль резюме для стажёрской/junior-вакансии:
- Тон скромный и конкретный: покажи фундамент и обучаемость, НЕ выставляй продакшн-экспертизу главным аргументом.
- Акцентируй: крепкий фундамент JS (типы, массивы, объекты, прототипы, AJAX), HTML/CSS, Git, пет-проекты.
- Буллеты опыта/проектов — активные глаголы в прошедшем времени + цифры (если есть в данных).
- Не выдумывать проценты/цифры. Английский — "чтение технической документации".`
...
function vacancyTextOf(input: GenerateRequest): string | undefined {
  const v = input.vacancy
  if (!v) return undefined
  return v.source === "url" ? v.url : v.text?.trim() || undefined
}

function buildDefaultInstructions(input: GenerateRequest): string {
  const level = vacancyLevel(vacancyTextOf(input))
...
- Опыт должен быть реалистичным и соответствовать заявленным навыкам.`,
  ...
  const tone = level === "junior" ? JUNIOR_STYLE : SENIOR_STYLE

  const about =
    level === "junior"
      ? `"О себе" — секция paragraph, 2–3 предложения:
- (1) кто ты: крепкий фундамент JS (типы данных, массивы, объекты, прототипы, AJAX) и основной стек;
- (2) конкретный факт из данных — что сделал руками (пет-проект/учебный проект, цифра если есть);
- (3) мотивация: хочу систематизировать знания и вырасти. Без senior-пафоса и без выдумок про стаж.`
      : `"О себе" — секция paragraph, 2–3 предложения:
- (1) роль + стек; (2) конкретный результат с цифрой из данных; (3) привязка к вакансии.`
...
export async function generateCv(
  ...
  const system =
    "Ты — эксперт по составлению резюме под конкретную вакансию. Ты пишешь только валидный JSON по заданной схеме."
...
export async function generateCoverLetter(
  ...
  const level = vacancyLevel(vacancy)

  const vacancyBlock = vacancy
    ? `Вакансия:\n"""\n${vacancy}\n"""\n`
    : "Текст вакансии не задан — опирайся на название должности и компании.\n"

  const contactsBlock = ctx.contacts.join("; ") || "—"

  const tone =
    level === "junior"
      ? `Вакансия стажёрская/junior. Тон — скромный, без senior-пафоса: подчеркни фундамент (типы, прототипы, AJAX, Git), пет-проекты и мотивацию систематизировать знания. Не выставляй продакшн-экспертизу главным аргументом.`
      : `Тон — уверенный, по делу: покажи ценность и конкретный результат с цифрой.`
...
Профиль кандидата (сжатые факты):
"""
${ctx.profile}
"""
...
- Объём — НЕ более 130 слов, читается за 20 секунд.
- Структура: приветствие «Здравствуйте!» (имя рекрутера неизвестно — не выдумывай его) → 1 короткий абзац (кто ты и на какую позицию откликаешься) → 1 абзац (2–3 конкретных факта, закрывающих требования вакансии) → 1–2 строки (готовность к диалогу и призыв к действию).
- НЕ дублируй резюме и НЕ перечисляй весь стек — выбери только 2–3 сильных факта, релевантных вакансии.
- Используй только реальные факты из профиля. НЕ выдумывай стаж, цифры, компании, технологии.
- Без «воды», канцелярита и фраз «я ищу себя», «очень хочу».
- ${tone}
- Подпись: ${ctx.name}${ctx.title ? `, ${ctx.title}` : ""}, затем контакты (телефон, email, telegram, github, сайт — что есть в данных), каждый с новой строки.
